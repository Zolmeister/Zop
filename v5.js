...
onmouseup = ontouchend = function (e) {
  ...
  for (var i = 0; i < selected.length; i++) {
    ...
    if (dot.r >= 0) {
      ...
      dot.color = colors[Math.random()*5|0]
    }
  }
    ...
    for (var i = 0; i < dots.length; i++) {
      ...
      if (selected.length &&
        (selected[0].color != dot.color ||
          Math.abs(
            Math.abs(selected[0].r - dot.r) - Math.abs(selected[0].c - dot.c)
          ) != 1))
        continue
      if (Math.abs(mouseX - dot.x) < dotSize / 2 &&
             Math.abs(mouseY - dot.y) < dotSize / 2) {
